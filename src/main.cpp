...
uint256 hashGenesisBlock("0xa9170a3b1f2a3504d5f5ea7fec9a150f633696031fcc8528f84b1d410ffbb48c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Frycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Frycoin Signed Message:\n";
...
bool CTxOut::IsDust() const {
	// Frycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Frycoin: 1 days
static const int64 nTargetSpacing = 2 * 60; // Frycoin: 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
	...
	// Frycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const {
	...
	// Frycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
//Following is from DarkCoin, Check their github - evan@darkcoin.io
		...
		if (nVersion < 2) {
			if ((!fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 950, 1000)) || (fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 75, 100))) {
		...
		if (nVersion >= 2) {
			...
			if ((!fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 750, 1000)) || (fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 51, 100))) {
...
bool CBlockIndex::IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned int nRequired, unsigned int nToCheck) {
	// Frycoin: temporarily disable v2 block lockin until we are ready for v2 transition
	return false;
...
bool InitBlockIndex() {
	...
	if (!fReindex) {
		...
		// Genesis block
		const char* pszTimestamp = "Who cares? Shut up and take my money!";
		CTransaction txNew;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
...
//		}
		...
		assert(block.hashMerkleRoot == uint256("0x83c4288f2dfb09e2001eea273d81934fb860b48196c0e30c970d96335534a06d"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Frycoin: increase each by adding 2 to bitcoin's value.
	...
	if (strCommand == "version") {
		...
		BOOST_FOREACH(CAddress& addr, vAddr) {
			...
			if (tx.AcceptToMemoryPool(state, true, true, &fMissingInputs)) {
				...
				for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
					uint256 hashPrev = vWorkQueue[i];
					for (set<uint256>::iterator mi = mapOrphanTransactionsByPrev[hashPrev].begin(); mi != mapOrphanTransactionsByPrev[hashPrev].end(); ++mi) {
			...
			BOOST_FOREACH(uint256& hash, vtxid) {
				...
				if ((pfrom->pfilter && pfrom->pfilter->IsRelevantAndUpdate(mempool.lookup(hash), hash)) || (!pfrom->pfilter))
					vInv.push_back(inv);
					if (vInv.size() == MAX_INV_SZ)
						break;
			}
		...
		} else if (!fBloomFilters && (strCommand == "filterload" || strCommand == "filteradd" || strCommand == "filterclear")) {
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn) {
	...
	int64 nFees = 0;
	{
		...
		while (!vecPriority.empty()) {
			...
			// transactions:
			if (!fSortedByFee && ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 576 / 250))) {
				fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey) {
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != hashBestChain)
			return error("FrycoinMiner : generated block is stale");
		...
		if (!ProcessBlock(state, NULL, pblock))
			return error("FrycoinMiner : ProcessBlock, block not accepted");
	}

	return true;
}

void static FrycoinMiner(CWallet *pwallet) {
	printf("FrycoinMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("Frycoin-miner");
	...
	try {
		while (true) {
			...
			printf("Running FrycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//				}
		...
		printf("FrycoinMiner terminated\n");
		throw;
	}
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet) {
	...
	for (int i = 0; i < nThreads; i++)
		minerThreads->create_thread(boost::bind(&FrycoinMiner, pwallet));
}
